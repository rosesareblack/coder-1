...
A template for building AI-powered coding agents that supports Claude Code, OpenAI's Codex CLI, GitHub Copilot CLI, Cursor CLI, Google Gemini CLI, and opencode with secure, cloud-based sandboxes from [E2B](https://e2b.dev/) to automatically execute coding tasks on your repositories.

![Coding Agent Template Screenshot](screenshot.png)

## Deploy Your Own

You can deploy your own version of the coding agent template to any platform that supports Next.js. Here's a general guide:

1.  **Fork the repository.**
2.  **Set up a PostgreSQL database** (e.g., using Neon, Supabase, or your own instance) and get the connection URL.
3.  **Create an E2B account** at [e2b.dev](https://e2b.dev/) to get an API key for the sandboxing environment.
4.  **Configure Environment Variables** on your hosting platform (see [Local Development Setup](#local-development-setup) for the required variables).
...
- **E2B Secure Sandboxes**: Runs code in isolated, secure sandboxes built for AI agents.
...
- **Persistent Storage**: Tasks stored in a PostgreSQL database with Drizzle ORM
...
2. Set up a Postgres database and an E2B account.
...
The maximum duration setting controls how long the E2B sandbox will stay alive from the moment it's created. You can select timeouts ranging from 5 minutes to 5 hours.
...
3. **Sandbox Setup**: An E2B sandbox is created with your repository
...
- **E2B Sandbox**: Sandboxes are isolated but ensure you're not exposing sensitive data in logs or outputs.
