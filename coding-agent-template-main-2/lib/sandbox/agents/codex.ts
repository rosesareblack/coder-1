...
export async function executeCodexInSandbox(
  ...
  try {
    ...
    const versionTestResult = await sandbox.process.start({
      cmd: 'codex',
      args: ['--version'],
      env: {
...
`
    ...
    const configSetupProc = await sandbox.process.start({
      cmd: 'sh',
      args: ['-c', `mkdir -p ~/.codex && cat > ~/.codex/config.toml << 'EOF'\n${configToml}EOF`],
      env: {},
      cwd: PROJECT_DIR,
    })
    await configSetupProc.wait()
    ...
    const configCheckProc = await sandbox.process.start({
      cmd: 'test',
      args: ['-f', '~/.codex/config.toml'],
      env: { HOME: '/home/vercel-sandbox' },
      cwd: PROJECT_DIR,
    })
    const configCheckResult = await configCheckProc.wait()
    ...
    const lsDebugProc = await sandbox.process.start({ cmd: 'ls', args: ['-la'] })
    await lsDebugProc.wait()
    if (logger) {
      await logger.info('Current directory contents retrieved')
    }

    // Debug: Show current working directory
    const pwdProc = await sandbox.process.start({ cmd: 'pwd', args: [] })
    await pwdProc.wait()
    if (logger) {
