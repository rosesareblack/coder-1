  ...
  try {
    ...
    if (currentTask.sandboxId && currentTask.keepAlive) {
      try {
        ...
        console.log('Calling Sandbox.reconnect with sandboxId:', currentTask.sandboxId)
        const reconnectedSandbox = await Sandbox.reconnect(currentTask.sandboxId)
    ...
    if (!sandbox) {
      ...
      await db
        .update(tasks)
        .set({
          sandboxId: sandbox?.id || undefined,
