  ...
  try {
    ...
    if (mode === 'local') {
      ...
      try {
        ...
        const numstatProc = await sandbox.process.start({
          cmd: 'git',
          args: ['diff', '--numstat', compareRef],
          cwd: PROJECT_DIR,
        })
        const numstatResult = await numstatProc.wait()
        ...
        const filePromises = statusLines.map(async (line) => {
          ...
          if (
            ...
            try {
              const wcProc = await sandbox.process.start({
                cmd: 'wc',
                args: ['-l', filename],
                cwd: PROJECT_DIR,
              })
              const wcResult = await wcProc.wait()
      ...
      try {
        ...
        const findProc = await sandbox.process.start({
          cmd: 'find',
          args: [
            '.',
            '-type',
        ...
        const findResult = await findProc.wait()
        ...
        const findOutput = findResult.stdout
