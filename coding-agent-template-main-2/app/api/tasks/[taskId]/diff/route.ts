  ...
  try {
    ...
    if (mode === 'local') {
      ...
      try {
        ...
        const checkRemoteProc = await sandbox.process.start({
          cmd: 'git',
          args: ['rev-parse', '--verify', remoteBranchRef],
          cwd: PROJECT_DIR,
        })
        const checkRemoteResult = await checkRemoteProc.wait()
        ...
        if (!remoteBranchExists) {
          ...
          const oldContentProc = await sandbox.process.start({
            cmd: 'git',
            args: ['show', `HEAD:${filename}`],
            cwd: PROJECT_DIR,
          })
          const oldContentResult = await oldContentProc.wait()
          let oldContent = ''
          if (oldContentResult.exitCode === 0) {
            oldContent = oldContentResult.stdout
          }
          // File might not exist in HEAD (new file)

          // Get new content (working directory version)
          const newContentProc = await sandbox.process.start({
            cmd: 'cat',
            args: [filename],
            cwd: PROJECT_DIR,
          })
          const newContentResult = await newContentProc.wait()
          const newContent = newContentResult.exitCode === 0 ? newContentResult.stdout : ''
        ...
        const diffProc = await sandbox.process.start({
          cmd: 'git',
          args: ['diff', remoteBranchRef, filename],
          cwd: PROJECT_DIR,
        })
        const diffResult = await diffProc.wait()

        if (diffResult.exitCode !== 0) {
          const diffError = diffResult.stderr
