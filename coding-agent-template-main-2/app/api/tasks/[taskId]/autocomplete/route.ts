...
export async function POST(request: NextRequest, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    try {
      ...
      const pwdProc = await sandbox.process.start({ cmd: 'sh', args: ['-c', 'pwd'] })
      const pwdResult = await pwdProc.wait()
      let actualCwd = cwd || '/home/vercel-sandbox'

      try {
        const pwdOutput = pwdResult.stdout
      ...
      const lsProc = await sandbox.process.start({ cmd: 'sh', args: ['-c', lsCommand] })
      const result = await lsProc.wait()
